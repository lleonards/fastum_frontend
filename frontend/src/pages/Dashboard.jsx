...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!authLoading && user?.id) fetchMessages()
  }, [authLoading, user?.id])
    ...
    return (
      <div className="min-h-screen bg-mystical flex items-center justify-center">
        <div className="glass-card p-8 text-center">
          <p className="text-white/70">Não foi possível iniciar sua sessão.</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img
              src={fastumLogo}
              alt="Fastum"
              className="h-9 sm:h-10 w-auto drop-shadow"
            />
            <div>
          ...
          <div className="flex items-center gap-3">
            <motion.button
              ...
              className="btn-primary flex items-center gap-1 px-3 py-2 sm:px-6 sm:py-3 text-xs sm:text-sm"
            >
              <span>✏️</span><span className="hidden xs:inline sm:inline"> Nova Mensagem</span>
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={handleSignOut}
              className="btn-secondary flex items-center gap-1 px-3 py-2 sm:px-6 sm:py-3 text-xs sm:text-sm"
            >
              <span>🚪</span><span className="hidden xs:inline sm:inline"> Sair</span>
            </motion.button>
          </div>
        </div>
      </header>

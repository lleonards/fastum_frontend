...
export default function Editor() {
  ...
  // Fecha dropdown de fonte ao clicar fora
  useEffect(() => {
  ...
  const loadMessage = async () => {
    ...
    setForm({
      ...
      box_color: data.box_color || '#c0392b',
    ...
    return (
      <div className="min-h-screen bg-mystical flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/50 text-sm">Iniciando sessão...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-mystical overflow-x-hidden">
      {/* Header */}
      <header className="border-b border-white/10 bg-black/30 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between gap-2 overflow-hidden">
          <div className="flex items-center gap-1 sm:gap-3 min-w-0 flex-1">
            <button onClick={() => navigate('/dashboard')} className="text-white/60 hover:text-white transition-colors flex-shrink-0 text-sm sm:text-base">
            ...
            <input
              ...
              placeholder="Título..."
              className="bg-transparent border-none outline-none text-white font-playfair text-lg placeholder-white/30 w-24 sm:w-48 min-w-0"
            />
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="btn-secondary text-sm flex items-center gap-1 px-3 py-2 sm:px-6 sm:py-3"
            >
            ...
            <motion.button
              ...
              onClick={handleSave}
              disabled={saving}
              className="btn-primary flex items-center gap-1 text-sm px-3 py-2 sm:px-6 sm:py-3"
            >
              {saving ? <><div className="spinner !w-4 !h-4" /><span className="hidden sm:inline"> Salvando...</span></> : <><span>🔗</span><span className="hidden sm:inline"> Gerar Link</span></>}
            </motion.button>
          </div>
        </div>
      </header>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                {form.theme === 'gift' && (
                  <div className="space-y-3">
                    {/* Cor da Caixa */}
                    <div className="glass-card p-4">
                      <label className="text-white/70 text-sm block mb-3">🎁 Cor da Caixa do Presente</label>
                      <div className="flex gap-2 flex-wrap mb-3">
                        {['#c0392b', '#8e44ad', '#2980b9', '#f39c12', '#27ae60', '#e91e8c', '#ff6b35', '#16a085', '#2c3e50', '#d35400'].map(c => (
                          <button
                            key={c}
                            onClick={() => setForm({ ...form, box_color: c })}
                            className={`w-8 h-8 rounded-full border-2 transition-all ${form.box_color === c ? 'border-amber-400 scale-110' : 'border-transparent'}`}
                            style={{ background: c }}
                          />
                        ))}
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="color"
                          value={form.box_color}
                          onChange={e => setForm({ ...form, box_color: e.target.value })}
                          className="w-10 h-8 rounded cursor-pointer bg-transparent border border-white/20"
                        />
                        <span className="text-white/40 text-xs">{form.box_color}</span>
                      </div>
                    </div>
                    {/* Cor da Fita */}
                    <div className="glass-card p-4">
                      <label className="text-white/70 text-sm block mb-3">🎀 Cor da Fita do Presente</label>
                      <div className="flex gap-2 flex-wrap mb-3">
                        {['#c0392b', '#8e44ad', '#2980b9', '#f39c12', '#27ae60', '#e91e8c', '#ff6b35', '#f1c40f', '#1abc9c', '#e67e22'].map(c => (
                          <button
                            key={c}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={e => setForm({ ...form, ribbon_color: e.target.value })}
                          className="w-10 h-8 rounded cursor-pointer bg-transparent border border-white/20"
                        />
                        <span className="text-white/40 text-xs">{form.ribbon_color}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {activeTab === 'style' && (
              <motion.div
                ...
                className="space-y-4"
              >
                {/* Fonte — Dropdown Customizado */}
                <div className="glass-card p-4 relative z-20">
                  <label className="text-white/70 text-sm block mb-3">Fonte do Texto</label>

                  {/* Botão disparador do dropdown */}
                  <div className="relative" ref={fontDropdownRef}>
                    <button
                      type="button"
                      onClick={() => setFontDropdownOpen(prev => !prev)}
                      className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white outline-none focus:border-amber-500/60 transition-all flex items-center justify-between gap-2"
                      style={{ fontFamily: form.font_family }}
                    >
                      <span style={{ fontFamily: form.font_family }}>
                        {FONTS.find(f => f.value === form.font_family)?.label || 'Selecione uma fonte'}
                      </span>
                      <span className="text-white/40 text-xs ml-auto">{fontDropdownOpen ? '▲' : '▼'}</span>
                    </button>

                    {/* Lista de opções customizada */}
                    {fontDropdownOpen && (
                      <div className="absolute top-full left-0 right-0 mt-1 rounded-xl overflow-hidden shadow-2xl border border-white/20"
                           style={{ background: '#1a1a2e', maxHeight: '280px', overflowY: 'auto', zIndex: 9999 }}>
                        {FONTS.map(f => (
                          <button
                            key={f.value}
                            type="button"
                            ...
                            className="w-full text-left px-4 py-3 transition-all hover:bg-amber-400/15"
                            style={{
                              fontFamily: f.value,
                              fontSize: '15px',
                              color: form.font_family === f.value ? '#f59e0b' : '#e8e8e8',
                              background: form.font_family === f.value ? 'rgba(245,158,11,0.12)' : 'transparent',
                  ...
                  {/* Prévia da fonte sobre o fundo do papel selecionado */}
                  <div
                    className="mt-3 p-4 rounded-xl text-center text-lg border border-white/10 shadow-inner"
                    style={{
                      background: PAPER_BG_MAP[form.paper_color] || PAPER_BG_MAP.parchment,
...
function PreviewCard({ theme, text, fontFamily, fontSize, textColor, paperBg, designClass, ribbonColor, boxColor, envelopeColor, photosCount }) {
  ...
  }, [theme, ribbonColor, boxColor])

  // Mantém ótimo no celular (max-w-xs) e cresce no desktop
  const previewWrapperClass = "w-full max-w-xs sm:max-w-[420px] md:max-w-[520px] lg:max-w-[640px] xl:max-w-[720px]";
        ...
        <AnimatePresence mode="wait">
          {!opened ? (
            ...
            >
              ...
              <div
                className={`p-6 md:p-8 min-h-[250px] md:min-h-[340px] relative ${designClass}`}
          ...
          {!opened ? (
            ...
            <motion.div key="letter-open" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <motion.div
                ...
                className={`p-5 md:p-7 rounded-lg shadow-xl ${designClass}`}
        ...
        <AnimatePresence mode="wait">
          {!opened ? (
            <motion.div
              ...
              onClick={() => setOpened(true)}
            >
              {/* Tampa do presente - usa boxColor como base */}
              <div
                className="relative rounded-t-lg py-5 px-6 text-center overflow-hidden"
                style={{
                  background: `linear-gradient(135deg, ${boxColor}cc 0%, ${boxColor}99 100%)`,
                  boxShadow: `0 -4px 15px ${boxColor}40`,
                }}
              >
                {/* Fita horizontal na tampa */}
                <div className="absolute inset-y-0 left-0 right-0 flex items-center">
                  <div className="w-full h-3" style={{ background: ribbonColor, opacity: 0.9 }} />
                </div>
                {/* Fita vertical na tampa */}
                <div className="absolute inset-0 flex justify-center">
                  <div className="w-3 h-full" style={{ background: ribbonColor, opacity: 0.9 }} />
                </div>
                {/* Laço SVG fiel ao original */}
                <div className="relative z-10 flex justify-center">
                  <motion.div
                    animate={{ rotate: [0, 5, -5, 0], scale: [1, 1.1, 1] }}
                    transition={{ duration: 2, repeat: Infinity }}
                  >
                    <svg width="64" height="48" viewBox="0 0 80 60">
                      <ellipse cx="20" cy="30" rx="18" ry="12" fill={ribbonColor} opacity="0.95" transform="rotate(-20 20 30)" />
                      <ellipse cx="60" cy="30" rx="18" ry="12" fill={ribbonColor} opacity="0.95" transform="rotate(20 60 30)" />
                      <ellipse cx="40" cy="30" rx="8" ry="8" fill={ribbonColor} />
                      <ellipse cx="40" cy="30" rx="5" ry="5" fill="rgba(255,255,255,0.35)" />
                      <path d="M35 36 Q30 50 25 58" stroke={ribbonColor} strokeWidth="4" fill="none" strokeLinecap="round" />
                      <path d="M45 36 Q50 50 55 58" stroke={ribbonColor} strokeWidth="4" fill="none" strokeLinecap="round" />
                    </svg>
                  </motion.div>
                </div>
              </div>
              {/* Corpo da caixa */}
              <div
                className="relative rounded-b-lg py-8 px-6 text-center overflow-hidden"
                style={{
                  background: `linear-gradient(135deg, ${boxColor}bb 0%, ${boxColor}88 100%)`,
                  boxShadow: `0 15px 30px rgba(0,0,0,0.35)`,
                }}
              >
                {/* Fita vertical no corpo */}
                <div className="absolute inset-0 flex justify-center pointer-events-none">
                  <div className="w-3 h-full" style={{ background: ribbonColor, opacity: 0.85 }} />
                </div>
                <p className="text-white/80 text-xs relative z-10 tracking-wider">🎁 Toque para abrir!</p>
              </div>
            </motion.div>
          ) : (
            <motion.div key="gift-open">
              ...
              <motion.div
                ...
                className="rounded-t-lg py-5 px-6 text-center overflow-hidden"
                style={{ background: `linear-gradient(135deg, ${boxColor}cc 0%, ${boxColor}99 100%)` }}
              >
              ...
              >
                <div
                  className={`p-5 md:p-7 rounded-b-lg shadow-xl ${designClass}`}
